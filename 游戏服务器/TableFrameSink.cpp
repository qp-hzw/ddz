...
void CTableFrameSink::OnUserPublicBet(WORD wChairID)
{
	...
	if (Rule == 0)
	{
		for (int i = 0; i < Rob_Num - 1; i++)    //¼ÆËãÇÀ×¯µÄ±¶Êý
		{
			Rob_bet *= 2;    // 2µÄ4´Î·½±¶
		}
	}
	...
	if (Boom_Num == 0)
		if (Mingpai_Bet == 0)
			public_Bet = cell_score *Rob_bet * LeaveCard_Bet* Spring_Bet;  //¹«¹²±¶Êý = ³õÊ¼*Ã÷ÅÆ*ÇÀµØÖ÷*µ×ÅÆ*Õ¨µ¯*´ºÌì
		else
			public_Bet = cell_score *Rob_bet * Mingpai_Bet * LeaveCard_Bet * Spring_Bet;
	else
		if (Mingpai_Bet == 0)
			public_Bet = cell_score *Rob_bet  * Boom_Bet * LeaveCard_Bet * Spring_Bet;  //¹«¹²±¶Êý = ³õÊ¼*Ã÷ÅÆ*ÇÀµØÖ÷*µ×ÅÆ*Õ¨µ¯8´ºÌì
		else
