...
void CTableFrameSink::StartGame()
{
	...
	//Èç¹ûÊÇ±ÈÈü³¡µÄµÚÒ»¾Ö£¬¸üĞÂÉÏÒ»´ó¾ÖµÄ»ı·Ö
...
void CTableFrameSink::HandleDeal()
{
	...
	for (int i = 0; i < m_GameAccess->GetCurPlayerCount(); i++)
	{
		m_GameLogic->Wager(i, 15);    //Ä¿Ç°Ä¬ÈÏÎª15   ÒÔºó¹À¼ÆÒª¸Ä
	}
...
// ´¦Àí¿Í»§¶Ë·¢À´µÄ¡¾ÇÀ×¯¡¿ÏûÏ¢
void CTableFrameSink::OnUserCallBanker(WORD wChairID, BYTE cbResult)	//0-²»½Ğ  1-½ĞµØÖ÷  2-²»ÇÀ  3-ÇÀµØÖ÷
{
	printf("\n¡¾·şÎñÆ÷¡¿£º½ÓÊÕÓÃ»§ = %d, ÇÀ×¯ÀàĞÍ = %d\n", wChairID, cbResult);
	...
	else	//ÇÀ×¯»¹Î´Íê³É£¬Ôò¼ÌĞø¸øÏÂ¸öÍæ¼Ò·¢ËÍÇÀ×¯ÏûÏ¢
	{
		...
		{
			if ((USER_PLAYING == m_GameAccess->GetPlayerState(i)) &&
				(ROB_STATE_BUJIAO == m_GameAccess->GetBankerState(i)))     //0-²»½Ğ  1-½ĞµØÖ÷  2-²»ÇÀ  3-ÇÀµØÖ÷
			{
				cbPassNum++;
			}
		}
		...
		else	//¸øÏÂ¸öÍæ¼Ò·¢ËÍÇÀ×¯ÏûÏ¢
		{
			...
			while (wNextUser != wChairID)		//ÏÂ¸öÍæ¼Ò²»ÊÇ¡°¹ı¡±×´Ì¬²Å¸øËû·¢ËÍÇÀµØÖ÷ÏûÏ¢
			{
				...
				else if ((int)cbPassNum == 2 && robnum == 2)
				{
					...
					|| 10 == m_GameAccess->GetBankerState(wNextUser))	//³õÊ¼×´Ì¬
			...
			if (wNextUser == wChairID)
			{
				...
				for (int i = 0; i < cbMaxChairCount; i++)
				{
					if (ROB_TYPE_ROB == m_GameAccess->GetBankerState(wNextUser)
						|| ROB_TYPE_CALL == m_GameAccess->GetBankerState(wNextUser))  //0-²»½Ğ  1-½ĞµØÖ÷  2-²»ÇÀ  3-ÇÀµØÖ÷
					{
						cbRobUser = i;
					}
				}
...
void CTableFrameSink::HandleSendLeaveCard(const WORD &wSendCardUser)
{
	...
	if (0 < m_GameAccess->GetLaiZiMode())
	{
		...
		cout << "ñ®×Ó" << (int)m_GameAccess->GetMagicCard() << endl;
	}
