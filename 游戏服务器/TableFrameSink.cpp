...
bool CTableFrameSink::Initialization(ITableFrame *pTableFrame, tagTableRule *comRule)
{
	...
	//m_pITableFrame->SetStartMode(START_MODE_FULL_READY);	//ËùÓÐÈË×¼±¸¿ªÊ¼
	...
	// ¼ÓÔØ¶ÁÈ¡µÄÅäÖÃÎÄ¼þ²ÎÊýºÍ´´½¨ÈÕÖ¾,ÌúÁëÂé½«-¾­µäÄ£Ê½		
...
VOID  CTableFrameSink::RepositionSink()
{
	...
	{
		m_GameAccess->SetGameStatus( GAME_STATUS_FREE ); // µÈ´ý¿ªÊ¼
	}
}
...
bool CTableFrameSink::XjGameConclude(int nTotalGameCount, int nCurGameCount)
{
	...
	//¼ÇÂ¼Õû´ó¾ÖÖÐµ±Ç°Ð¡¾ÖµÄµÃ·ÖÊÇ²»ÊÇ×î´ó
	for (BYTE i = 0; i < _playersum && m_GameAccess != NULL; ++i)
	{
		SCORE CurBestBet = m_GameAccess->GetAllBet(i);
		if (m_GameAccess->GetDJPlayerBestBet(i) < CurBestBet)      //ÉÏÒ»¾ÖµÄ·Ö±ÈÕâÒ»¾ÖÐ¡ ¾ÍÌæ»»
		{
	...
	for (BYTE i = 0; i < _playersum && m_GameAccess != NULL; ++i)
	{
		if (USER_PLAYING == m_GameAccess->GetPlayerState(i))
		{
			//Ö±½Ó½«Íæ¼Ò×Ü»ý·Ö·¢¸ø¿Í»§¶Ë£¬±ÜÃâ¿Í»§¶Ë¼ÆËã
			...
			if ((int)info.m_goldCoin < (int)(-msg_singleGameScore[BankID]))
			{
				for (int i = 0; i < _playersum; i++)
				{
					if (i != BankID)
					{
						...
						//Å©Ãñ×î¶àÖ»ÄÜÓ®ËûµÄ½ð±ÒÄÇÃ´¶à½ð±Ò
	...
	for (BYTE j = 0; j < _playersum && m_GameAccess != NULL; j++)
	{
		...
		for (BYTE i = 0; i < _playersum && m_GameAccess != NULL; i++)
		{
			...
			sXJGameEnd.xj_end[i].UserBet = m_GameAccess->GetAllBet(i);			//¸÷Íæ¼Ò±¶Êý
...
bool CTableFrameSink::DjGameConclude(int nTotalGameCount, int nCurGameCount)
{
	...
	for (BYTE i = 0; i < _playersum; ++i)
	{
		if (USER_PLAYING == m_GameAccess->GetPlayerState(i))
		{
			...
			//player_info[i] = m_GameAccess->GetPlayerInfo(i);	//Íæ¼ÒÐÅÏ¢

			m_GameAccess->GetWinSum(i, msg_winTime[i]);					//Ó®´ÎÊý
			//msg_bankTime[i] = m_GameAccess->GetPlayerBankCount(i);		//×ö×¯´ÎÊý
			//msg_BestBet[i] = m_GameAccess->GetDJPlayerBestBet(i);		//×î´ó±¶Êý
	...
	for (int i = 0; i < _playersum; i++)
	{
		...
		//sDJGameEnd.bWinTime[i] = msg_winTime[i];     //Ó®´ÎÊý
		//sDJGameEnd.BankTime[i] = msg_bankTime[i];	//µØÖ÷´ÎÊý
		//sDJGameEnd.PlayerBigestBet[i] = msg_BestBet[i];	 //¼ÇÂ¼×î´ó±¶Êý
...
bool CTableFrameSink::OnEventGameConclude(BYTE cbReason)
{
	...
	case GER_NORMAL:			//Õý³£½áÊø
		{
			if (cbCurGameCount == cbAllGameCount || 2 == m_pITableFrame->GameType())		// ´ó¾Ö½áÊø		//½ð±Ò³¡ÅÐ¶ÏÕâÀï¼ÓÉÏ
			{
...
//			statusAddStore.replay_code = m_pITableFrame->GetRoomBaseInfo().replay_code;
		...
		case GS_WK_OUT_CARD:	//³öÅÆ×´Ì¬
		{
			...
			//Èç¹ûÓÃ»§ÖØÁ¬ÉÏÁË Çå³ýËûµÄÁ¬Ðø³¬Ê±´ÎÊý
			...
			StatusOutCard.Leave_bet = m_GameAccess->GetLeaveCardBet();  //µ×ÅÆ±¶Êý

			StatusOutCard.CurJuShu = m_GameAccess->GetCurGameCount();   //ÓÎÏ·¾ÖÊý
			StatusOutCard.bankID = m_GameLogic->GetAppointBanker();   //×¯¼Ò
//			StatusOutCard.replay_code = m_pITableFrame->GetRoomBaseInfo().replay_code;
			StatusOutCard.room_bet = m_GameAccess->GetAllBet(wChairID);   //·¿¼ä±¶Êý
...
bool CTableFrameSink::OnTimerMessage(DWORD wTimerID, WPARAM wBindParam)
{
	...
	switch(wTimerID)
	{
		case IDI_ROB_BANKER: // ÇÀ×¯¶¨Ê±Æ÷
		{
			...
			//Ã»ÓÐ½Ð×¯,Ä¬ÈÏ²»½Ð   Èç¹ûÊÇ»úÆ÷ÈË£¬Ëæ»ú½Ð×¯
		...
		case IDI_XJGAME_USER_READY:  // Ð¡¾Ö½áÊøÓÃ»§×¼±¸¶¨Ê±Æ÷
		{
			m_pITableFrame->KillGameTimer(IDI_XJGAME_USER_READY);
			printf("\n¡¾·þÎñÆ÷¡¿£º¶¨Ê±Æ÷µ½£¬½øÈë¡¾Ð¡¾Ö¿ªÊ¼¡¿\n");
			//Ã»ÓÐ×¼±¸£¬ÔòÄ¬ÈÏÈ«²¿×¼±¸£¬¿ªÊ¼ÓÎÏ·
			for (WORD i = 0; i < m_GameAccess->GetMaxChairCount() && m_GameAccess != NULL; i++)
			{
				...
				{
					printf("\n¡¾·þÎñÆ÷¡¿£º¶¨Ê±Æ÷µ½£¬½ÓÊÜ¡¾Ð¡¾Ö¿ªÊ¼¡¿ Íæ¼Ò£º%d\n", i);
					//ÓÃ»§×¼±¸
		...
		case IDI_MING_PAI:		//Ã÷ÅÆ¶¨Ê±Æ÷
		{
			...
			//Ã»ÓÐÃ÷ÅÆ Ä¬ÈÏ²»Ã÷ÅÆ    »úÆ÷ÈËÒ²Ä¬ÈÏ²»Ã÷ÅÆ
			for (WORD i = 0; i < m_GameAccess->GetMaxChairCount() && m_GameAccess != NULL; i++)
			{
				if (USER_PLAYING == m_GameAccess->GetPlayerState(i)
					/*&& (0 == m_GameAccess->GetXjReadyState(i))*/
		...
		case IDI_OUT_CARD:		//³öÅÆ¶¨Ê±Æ÷
		{
			...
			{
				cout << "GS_WK_OUT_CARD != m_GameAccess->GetGameStatus() : err £ºÍÐ¹Ü" << endl;
				return false;
			}

			//Èç¹ûÍæ¼ÒÁ¬ÐøÈý´Î¶¨Ê±Æ÷³¬Ê± ÄÇÃ´½«ËûÉèÖÃ³ÉÍÐ¹Ü×´Ì¬
			...
			if (1 == m_GameAccess->GetOneTurnEnd())
			{
				...
				if (m_pITableFrame->IsRobot(OutCardUser))
				{
					...
					{
						On_Sub_UserOutCard(OutCardUser, NULL, 0, 1);   //³öÅÆ¹ý
						break;
					}
					else
					{
				...
				else
				{
					...
					if (m_GameLogic->AnalysePlayerOutCard(OutCardUser, OutCardData, &OutCardNum))
					{
						...
						{
							cout << "ÅÐ¶Ï Íæ¼Ò¿ÉÒÔ³öµÄÅÆ err" << endl;
							//CLog::Log(log_error, "func ÅÐ¶Ï Íæ¼Ò¿ÉÒÔ³öµÄÅÆ err : Ð£ÑéÊ§°Ü");
							return false;
						}
					...
					if (m_pITableFrame->IsRobot(i))
					{
						...
						for (int j = 0; j < m_GameAccess->GetCurPlayerCount(); j++)
						{
							if ((0 != m_GameAccess->GetCurJiaoFenState(j))
								&& (10 != m_GameAccess->GetCurJiaoFenState(j)))    //Ä¬ÈÏ×´Ì¬ºÍ²»½Ð×´Ì¬
							{
								m_Score -= 1 << (m_GameAccess->GetCurJiaoFenState(j));
							}
						}

						if (JIAOFEN_START == m_Score)  //ÁùÖÖ½Ð·ÖÇé¿ö
						{
...
bool CTableFrameSink::OnGameMessage(WORD wSubCmdID, VOID * pData, WORD wDataSize, WORD wChairID)
{
	...
	switch (wSubCmdID)
	{
		case SUB_CS_OUT_CARD:		//³öÅÆ
		{
			...
			//Èç¹ûÓÃ»§³öÅÆÁË Çå³ýËûµÄÁ¬Ðø³¬Ê±´ÎÊý
			m_GameAccess->SetPlayerTuoGuan(wChairID, 0);
		...
		case SUB_CS_XJ_GAME_READY:       //Íæ¼ÒÐ¡½áËã×¼±¸
		{
			//Ð£ÑéÊý¾Ý
		...
		case SUB_CS_PUBLIC_BET:		//ÓÎÏ·½áÊøµÄ¹«¹²±¶ÊýÐÅÏ¢
		{
			//Ð£ÑéÊý¾Ý
...
void CTableFrameSink::StartGame()
{
	...
	//Èç¹ûÊÇ±ÈÈü³¡µÄµÚÒ»¾Ö£¬¸üÐÂÉÏÒ»´ó¾ÖµÄ»ý·Ö
	...
	//ÐÂ²ßÂÔ£º·¢ÅÆÊ±ºòÑ¡ÔñÊÇ·ñÃ÷ÅÆºÍ¿ªÊ¼ÓÎÏ·Ã÷ÅÆ
	//ËùÒÔÕâÀï²»ÇÀ×¯
}
...
void CTableFrameSink::HandleDeal()
{
	...
	//for (int i = 0; i < m_GameAccess->GetCurPlayerCount(); i++)
	//{
	//	m_GameLogic->Wager(i, m_GameAccess->GetCellScore());    //Ä¿Ç°Ä¬ÈÏÎª15   ÒÔºó¹À¼ÆÒª¸Ä
	//}
...
void CTableFrameSink::SendJiaoFenStart(WORD wChairID, const BYTE &ActionType)
{
	...
	if (INVALID_CHAIR != wChairID)
	{
		...
		//ÅÐ¶Ï»úÆ÷ÈË
		if (m_pITableFrame->IsRobot(wChairID))     //´ýÌí¼Ó »úÆ÷ÈË½Ó¿Ú»¹Ã»Ìí¼Ó
		{
			WORD index = (rand() % 4) + 1;
...
void CTableFrameSink::OnUserJiaoFen(WORD wChairID, BYTE Score)
{
	...
	else
	{
		...
		//Èç¹ûËùÓÐÈË²»½Ð ÖØÐÂ·¢ÅÆ
		if (3 == cbPassNum)
		{
			//ÈýÂÖ¶¼²»½Ð Ëæ»úÒ»¸öÍæ¼ÒµØÖ÷
			...
			//Çå¿ÕÍæ¼ÒµÄ½Ð·Ö×´Ì¬  Çå¿ÕÍæ¼ÒµÄÃ÷ÅÆ×´Ì¬  Çå¿ÕÃ÷ÅÆ±¶Êý
			for (int i = 0; i < cbMaxChairCount; i++)
			{
		...
		{
			//·ñÔòÅÐ¶Ï½Ð·ÖÊÇ·ñÒÑ¾­½Ð¹ýÁËÈý´Î ÓÐÈý´ÎÉèÖÃ½Ð·Ö×î´óµÄÍæ¼ÒÎªµØÖ÷
			if (3 == JiaoNum)
			{
			...
			else		//·ñÔò ¸øÏÂ¸öÍæ¼Ò·¢ËÍ½Ð·ÖÐÅÏ¢
			{
				...
				for (int i = 0; i < m_GameAccess->GetCurPlayerCount(); i++)
				{
					if ((USER_PLAYING == m_GameAccess->GetPlayerState(i)) &&
						(0 != m_GameAccess->GetCurJiaoFenState(i))
						&& (10 != m_GameAccess->GetCurJiaoFenState(i)))    //Ä¬ÈÏ×´Ì¬ºÍ²»½Ð×´Ì¬
					{
...
{
	//µÚÒ»¾Ö£º´Ó·¿Ö÷¿ªÊ¼·¢ËÍÇÀ×¯¿ªÊ¼ÏûÏ¢	µÚ¶þ¾Ö£º´ÓÉÏ¾ÖµÄÓ®¼Ò¿ªÊ¼·¢ËÍÇÀ×¯ÏûÏ¢

	//³õÊ¼»¯Ä¬ÈÏµÄÍæ¼ÒÇÀ×¯×´Ì¬
	for (WORD i = 0; i < m_GameAccess->GetMaxChairCount(); i++)   //0ÊÇÄ¬ÈÏ×´Ì¬ 
	{
...
void CTableFrameSink::SendRobStart(const WORD &wChairID, const BYTE &cbType)
{
	...
	if (INVALID_CHAIR != wChairID)
	{
		...
		//ÅÐ¶ÏÊÇ²»ÊÇ»úÆ÷ÈË
		if (m_pITableFrame->IsRobot(wChairID))     //»úÆ÷ÈË½Ó¿Ú´ýÌí¼Ó
		{
			WORD index = (rand() % 3) + 1;
...
// ´¦Àí¿Í»§¶Ë·¢À´µÄ¡¾ÇÀ×¯¡¿ÏûÏ¢
void CTableFrameSink::OnUserCallBanker(WORD wChairID, BYTE cbResult)	//1-²»½Ð  2-½ÐµØÖ÷  3-²»ÇÀ  4-ÇÀµØÖ÷
{
	CLog::Log(log_debug, "¡¾·þÎñÆ÷¡¿£º½ÓÊÕÓÃ»§ = %d, ÇÀ×¯ÀàÐÍ = %d", wChairID, cbResult);
	...
	if (ROB_TYPE_ROB == cbResult)
	{
		...
		for (int i = 0; i < m_GameAccess->GetMaxChairCount(); i++)
		{
			if (USER_PLAYING == m_GameAccess->GetPlayerState(i))
			{
				m_GameLogic->Wager(i, 2);      //Èç¹ûÇÀµØÖ÷  ¸øÃ¿¸öÍæ¼Ò±¶Êý³Ë2
			}
		}
	}
	...
	if (2 == m_GameAccess->GetMaxChairCount())
	{
		...
		else
		{
			...
			SendRobStart(wNextUser, ROB_TYPE_CALL);    //½ÐµØÖ÷ÀàÐÍ
		}
		return;
	}
	////////////////////////////////////////////////////////////////////////

	//ÉèÖÃÒ»¸ö²ÎÊý¼ÇÂ¼ÇÀ×¯´ÎÊý      ÎªÁËÅÐ¶Ï¹ý¹ýÇÀµØÖ÷µÄÇé¿ö
	...
	else	//ÇÀ×¯»¹Î´Íê³É£¬Ôò¼ÌÐø¸øÏÂ¸öÍæ¼Ò·¢ËÍÇÀ×¯ÏûÏ¢
	{
		//ÅÐ¶ÏÊÇ²»ÊÇËùÓÐÍæ¼Ò¶¼Ñ¡Ôñ²»ÇÀ×¯
		BYTE cbMaxChairCount = m_GameAccess->GetMaxChairCount();
		BYTE cbPassNum = 0;
		for (int i = 0; i < cbMaxChairCount; i++)
		{
			if ((USER_PLAYING == m_GameAccess->GetPlayerState(i)) &&
				(ROB_STATE_BUJIAO == m_GameAccess->GetBankerState(i)))     //1-²»½Ð  2-½ÐµØÖ÷  3-²»ÇÀ  4-ÇÀµØÖ÷
			{
				cbPassNum++;
			}
		}

		//ËùÓÐÈË·ÅÆúÇÀ×¯£¬ÔòÖØÐÂ·¢ÅÆ
		if (cbPassNum == cbMaxChairCount)
		{
			cout << "ËùÓÐÈË·ÅÆúÇÀ×¯" << endl;
			...
			//Çå¿ÕÍæ¼ÒµÄÃ÷ÅÆ×´Ì¬ //Çå¿ÕÍæ¼ÒµÄÃ÷ÅÆ±¶Êý
			for (int i = 0; i < cbMaxChairCount; i++)
			{
				m_GameAccess->SetMingPaiState(i, 0);
				m_GameAccess->SetPlayerMingPaiBet(i, 1);
		...
		else	//¸øÏÂ¸öÍæ¼Ò·¢ËÍÇÀ×¯ÏûÏ¢
		{
			...
			//ÅÐ¶ÏÏÂ¸öÍæ¼ÒµÄÇÀ×¯×´Ì¬
			while (wNextUser != wChairID)		//ÏÂ¸öÍæ¼Ò²»ÊÇ¡°¹ý¡±×´Ì¬²Å¸øËû·¢ËÍÇÀµØÖ÷ÏûÏ¢
			{
			...
			{
				//Ä¿Ç°ÓÐÁ½¸öÍæ¼Ò¶¼²»ÇÀ »ñÈ¡µ±Ç°ÇÀ×¯µÄÍæ¼Ò  ÉèÖÃËûÎªµØÖ÷
				BYTE cbMaxChairCount = m_GameAccess->GetMaxChairCount();
				WORD cbRobUser;
				for (int i = 0; i < cbMaxChairCount; i++)
				{
					if (ROB_TYPE_ROB == m_GameAccess->GetBankerState(wNextUser)
						|| ROB_TYPE_CALL == m_GameAccess->GetBankerState(wNextUser))  //1-²»½Ð  2-½ÐµØÖ÷  3-²»ÇÀ  4-ÇÀµØÖ÷
					{
						cbRobUser = i;
					}
				}
...
void CTableFrameSink::AllRobBankerOver()
{
	...
	//·¢ËÍµ±Ç°·¿¼ä±¶Êý¸ø¸÷Íæ¼Ò						//¼ÇµÃ½Ð¿Í»§¶Ë¼ÓÉÏÕâ¸öÏûÏ¢
	STR_CMD_SC_FANGJIAN_BET  fangjian_bet;
	ZeroMemory(&fangjian_bet, sizeof(STR_CMD_SC_FANGJIAN_BET));

	//¸øµØÖ÷¼ÓÉÏÅ©ÃñµÄ±¶Êý  ²¢¸øÅ©Ãñ¼ÓÉÏµØÖ÷µÄ±¶Êý
	for (int i = 0; i < m_GameAccess->GetCurPlayerCount(); i++)
	{
		fangjian_bet.room_bet[i] = m_GameAccess->GetAllBet(i);
	}
...
void CTableFrameSink::HandleSendLeaveCard(const WORD &wSendCardUser)
{
	...
	for (WORD i = 0; i < cbMaxChairCount; i++)
	{
		if ((USER_PLAYING == m_GameAccess->GetPlayerState(i)))
		{
			...
			//µ×ÅÆÂ¼Ïñ
			//if (i == m_GameLogic->GetAppointBanker())    //´ýÌí¼Ó
...
void CTableFrameSink::SendAddScoreStart(const WORD &wChairID)
{
	...
	CLog::Log(log_debug, "¡¾·þÎñÆ÷¡¿£º¹ã²¥¡¾ÏÂ×¢¿ªÊ¼¡¿");
...
{
	CLog::Log(log_debug, "¡¾·þÎñÆ÷¡¿£º½ÓÊÕ¡¾ÏÂ×¢¡¿£¬Íæ¼Ò = %d£¬ÏÂ×¢ = %ld", wChairID, lScore);
	...
	//¸øµØÖ÷¼ÓÉÏÅ©ÃñµÄ±¶Êý		//¸øÅ©Ãñ¼ÓÉÏµØÖ÷µÄ¼Ó±¶
	...
	// ¼ÆËã¼Ó×¢Íê³ÉÍæ¼ÒÊý
	BYTE cbBetedNum = 0;
	BYTE cbPlayerNum = m_GameAccess->GetMaxChairCount();// »ñÈ¡Íæ¼ÒÊý
	for (BYTE i = 0; i < cbPlayerNum; ++i)
	{
...
void CTableFrameSink::SendMingPaiStart(const WORD &wChairID, const BYTE &cbMPType)
{
	...
	CLog::Log(log_debug, "¡¾·þÎñÆ÷¡¿£ºÍ¨ÖªÓÃ»§ = %d¡¾Ã÷ÅÆ¿ªÊ¼¡¿", wChairID);
...
//Ã÷ÅÆÊÂ¼þ
void CTableFrameSink::On_Sub_UserMingPai(WORD wChairID, const BYTE &cbMPType, BYTE cbFlag, BYTE FlushCardBet)     //Ã÷ÅÆÀàÐÍ	2-¿ªÊ¼ÓÎÏ·Ã÷ÅÆ	1-³öÅÆ¿ªÊ¼Ã÷ÅÆ  3-·¢ÅÆÃ÷ÅÆ
{																												//ÊÇ·ñÃ÷ÅÆ	0-²»Ã÷ÅÆ	1-Ã÷ÅÆ    //·¢ÅÆÃ÷ÅÆµÄ±¶Êý  4 - 3 - 2
	//ÉèÖÃÍæ¼ÒÃ÷ÅÆÍê³É
...
void CTableFrameSink::HandleOutCardStart(const WORD &wOutCardUser)
{
	...
	//ÅÐ¶Ïµ±Ç°³öÅÆ»ØºÏÊÇ·ñ½áÊø
	CLog::Log(log_debug, "ÅÐ¶Ïµ±Ç°»ØºÏÊÇ·ñ½áÊø: %d(0-½áÊø	1-Î´½áÊø)", m_GameAccess->GetOneTurnEnd());         //0001-³öÅÆ  0010-²»³ö  0100-ÌáÊ¾  1000-Òª²»Æð
...
{
	//Ð£Ñé ÊÇ·ñÊÇµ±Ç°Íæ¼Ò³öÅÆ
	if (wOutCardUser != m_GameAccess->GetCurOutCardUser())
		return;

	//ÈôÍæ¼ÒÊÇÒ»ÂÖÖÐ×îÏÈ¿ªÊ¼³öÅÆÕß
	...
	{
		//Ð£Ñé³öÅÆÊýÄ¿ÊÇ·ñºÍ´ËÂÖ×îºó³öÅÆÍæ¼ÒÏàÍ¬£¬³öÅÆÐ£Ñé
	...
	if (1 == cbFlag)
	{
		...
		else
		{
			...
			for (int i = 14; i >= 0; i--)
			{
				if ((cbCardType & (1 << i)) != 0)
				{
					OutCard.cbCardType = i;	//ÅÆÐÍ
					CLog::Log(log_debug, "ÅÆÐÍ: %d", OutCard.cbCardType);
					break;
				}
			}
...
{		
	...
	//ÉèÖÃ¿ªÊ¼µÄ³öÅÆÍæ¼Ò£¬Çå¿Õ¸ÃÂÖµÄ³öÅÆÊý¾Ý
	if ( 0 != m_GameAccess->GetUserCurCardNum(wWinner) )
	{
		...
		CLog::Log(log_debug, "¡¾Íæ¼Ò»¹ÓÐÅÆ£¬Ò»ÂÖ½áÊø£¬Í¨ÖªÓ®¼Ò = %d³öÅÆ¿ªÊ¼¡¿", wWinner);
	}
	else	//Íæ¼ÒËùÓÐÅÆ³öÍê
	{
...
void CTableFrameSink::OnUserPublicBet(WORD wChairID)
{
	...
	WORD LeaveCard_Bet = 1; //µ×ÅÆ±¶Êý
	WORD Spring_Bet = 1;	//´ºÌì±¶Êý
	WORD Mingpai_Bet = 1;  //Ã÷ÅÆ±¶Êý
	WORD Boom_Num = 0;   //Õ¨µ¯×ÜÊýÄ¿
	WORD Boom_Bet = 1;	 //Õ¨µ¯×Ü±¶Êý
	WORD Rob_Num = 0;    //ÇÀ×¯´ÎÊý
	WORD Rob_bet = 1;    //ÇÀ×¯±¶Êý
	WORD public_Bet = 0;  //¹«¹²±¶Êý
	WORD cell_score = m_GameAccess->GetCellScore();
	tagBoomInfo  boominfo;  //Õ¨µ¯ÐÅÏ¢
	...
	if (m_GameLogic->GetAppointBanker() < 0 || m_GameLogic->GetAppointBanker() > m_GameAccess->GetMaxChairCount())	//ÅÐ¶ÏÊÇ·ñÇÀÍêÁËµØÖ÷
	...
	for (int i = 0; i < m_GameAccess->GetCurPlayerCount(); i++)
	{
		public_bet.AddScore[i] = m_GameAccess->GetPlayerAddScore(i);    //¸÷Íæ¼ÒµÄ¼Ó±¶ÐÅÏ¢
		Boom_Num += m_GameAccess->GetUserBoomInfo(i, boominfo);			//»ñÈ¡Õ¨µ¯×ÜÊý
		Rob_Num += m_GameAccess->GetUserRobNum(i);						//»ñÈ¡×Ü¹²ÇÀ×¯´ÎÊý
	}

	//ÓÉ¹æÔòÀ´ÅÐ¶ÏÇÀ×¯²¿·ÖµÄ±¶Êý
	if (Rule == 0)
	{
		for (int i = 0; i < Rob_Num; i++)    //¼ÆËãÇÀ×¯µÄ±¶Êý
		{
			Rob_bet *= 2;    // 2µÄ4´Î·½±¶
		}
	}
	else		//½Ð·Ö±¶ÊýËã×î´óµÄÄÇÒ»¸ö
	{
		for (int i = 0; i < _playernum; i++)
		{
	...
	public_bet.Rob = Rob_bet;	//ÇÀµØÖ÷»ò½Ð·Ö
	...
	if (Boom_Num == 0)
		if (Mingpai_Bet == 0)
			public_Bet = Rob_bet * LeaveCard_Bet* Spring_Bet;  //¹«¹²±¶Êý = ³õÊ¼*Ã÷ÅÆ*ÇÀµØÖ÷*µ×ÅÆ*Õ¨µ¯*´ºÌì
		else
			public_Bet = Rob_bet * Mingpai_Bet * LeaveCard_Bet * Spring_Bet;
	else
		if (Mingpai_Bet == 0)
			public_Bet = Rob_bet  * Boom_Bet * LeaveCard_Bet * Spring_Bet;  //¹«¹²±¶Êý = ³õÊ¼*Ã÷ÅÆ*ÇÀµØÖ÷*µ×ÅÆ*Õ¨µ¯8´ºÌì
		else
...
/*************************************************
@Description:     ³õÊ¼»¯Â¼ÏñÊý×é£¬»ñµÃ¸öÊý¾ÝµÄË÷Òý¼°´óÐ¡
@Input:           ÎÞ
@Output:          ÎÞ
@Return:          ÎÞ
...
//@Input:           ÎÞ
//@Output:          ÎÞ
//@Return: lD#ûXkQ?ävÐÝ0Õ„9Œ®E?/@ ¿bözþ™?øbj?““‹‰F?p_½ç´HK5?2?os+ÁcŽn-?$? qÃúw?æxlšIÔú??ªS¯ÞbˆÏŠ£Ïõâ¡^bcVI™H“Â?Ç§"p8ÝÒÅÀÇ¿ªw?=Ó²ñ[±WºÝˆáŒ‚2Va“êDÑÏ.ŒÐ°@??ÿ½Ú6¶‘pÅxüÅz
//`t?ŒoPT<~0M9¾÷
//g
